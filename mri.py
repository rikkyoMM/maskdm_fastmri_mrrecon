...
# 画像のロード
    ...
    for ckpt in args.ckpt:
        ...
        def denoise_image(diffusion_model, noisy_image, t):
            ...
            :return: デノイズ化された画像。
            """
            # for i in reversed(range(t.item())):  # t.item() でスカラー値を取得
            for i in reversed(range(250)):  # t.item() でスカラー値を取得
        ...
        t = torch.tensor([int((diffusion_model.num_timesteps-1) * 0.01)], device=device)
        # print(t)
        ...
        denoised_image = diffusion_model.ddim_sample_gpt(noisy_image, 100)
