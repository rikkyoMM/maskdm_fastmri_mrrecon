...
#     steps = timesteps + 1
#     x = torch.linspace(0, timesteps, steps, dtype = torch.float64)
#     alphas_cumprod = torch.cos(((x / timesteps) + s) / (1 + s) * math.pi * 0.5) ** 2 / 4 + 0.75
...
def cosine_beta_schedule(timesteps, s = 0.008, shift=False, d=64):
    ...
    betas_alphas_df.to_csv("./cosschedule.csv", index=False)
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample_iws(self, noisy_image, guidance_weight=-1, guidance_idx=None, use_corrector=False):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = noisy_image.shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
    ...
    @torch.no_grad()
    def ddim_sample_gpt(self, noisy_image, start_timestep, guidance_weight=-1, guidance_idx=None, use_corrector=False):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = noisy_image.shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        # 元コード改造
        # times = torch.linspace(total_timesteps - start_timestep-1, total_timesteps-1, steps=start_timestep + 1)
        # print(times)

        times = torch.linspace(-1, start_timestep - 1, steps=start_timestep + 1)
    ...
    def forward(self, batch, *args, **kwargs):
        ...
        # print(t)
        # # タイムステップが500の場合の処理を追加
        # if t.max() == 995:  # タイムステップは0から始まるため、最大値は499
        #     self.save_images(x_start, noise, t, "./500step.jpg")
...
# gptに作らせた
    def save_images(self, x_start, noise, t, file_path):
        with torch.no_grad():
            noisy_img = self.q_sample(x_start, t, noise)
            # pdb.set_trace()

            self.save_images_grid(noisy_img, './500step_noise.jpg')
            self.save_images_grid(x_start, './500step_start.jpg')
            # 画像を保存
            # self.saveimg(x_start[0], file_path.replace('.jpg', '_x_start.jpg'))
            # self.saveimg(noisy_img[0], file_path.replace('.jpg', '_noisy_img.jpg'))
    ...
    #     image_np = img_np[0, ...]  # チャネル次元を除去
        

    #     image = Image.fromarray(image_np, 'L')  # グレースケールイメージとして読み込み
    #     image.save(filename)

    def save_images_grid(self, images, file_path):
        with torch.no_grad():
            # 正規化: 最小値を0、最大値を1に
            ...
            # 画像をグリッドに配置
            grid_img = make_grid(images_np, nrow=8)  # 8列のグリッド

            # numpy配列からPILイメージに変換
            grid_image = Image.fromarray(grid_img, 'L')  # グレースケールイメージとして読み込み
            grid_image.save(file_path)



def make_grid(images_np, nrow=8):
        # グリッドのサイズ計算
    nimg = images_np.shape[0]
    ncol = int(np.ceil(nimg / nrow))

        # 各画像のサイズ
    img_h, img_w = images_np.shape[2], images_np.shape[3]

        # グリッド画像の初期化
    grid_img = np.zeros((ncol * img_h, nrow * img_w), dtype=np.uint8)

        # 画像をグリッドに配置
    for i, img in enumerate(images_np):
        row = i // nrow
        col = i % nrow
        grid_img[row*img_h:(row+1)*img_h, col*img_w:(col+1)*img_w] = img[0, ...]  # チャネル次元を除去
